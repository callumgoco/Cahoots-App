-- Phase 1: revoke peer-readable apple_subject_id and validate clip storage paths.
-- Note: table-level SELECT includes all columns; column REVOKE alone is insufficient.
-- Follow-up migration 20260909201000 applies the effective column grants.

revoke select (apple_subject_id) on table public.profiles from anon, authenticated;
...
begin
  ...
  loop
    ...
    if coalesce((clip->>'durationSeconds')::numeric, 0) < 2
       ...
       or storage_path = ''
       or storage_path !~ ('^' || expected_prefix || '[^/]+\.mov$')
       or not exists (
