...
enum CahootsDomainEvent: Equatable, Sendable {
...
enum CahootsStateReconciler {
    ...
    private static func makeResult(snapshot: DemoSnapshot, challenge: CahootsChallenge, completedAt: Date) -> CahootsResult {
        ...
        return .init(
            ...
            title: challenge.title, winnerName: entries.first?.user.displayName ?? "Cahoots members",
...
enum ChallengeFactory {
    static func makeChallenge(from proposal: ChallengeProposal, createdAt: Date, id: UUID = UUID()) -> CahootsChallenge {
        CahootsChallenge(
            id: id,
    ...
    static func seedLeaderboardEntries(for challenge: CahootsChallenge, in snapshot: inout DemoSnapshot, at now: Date) {
