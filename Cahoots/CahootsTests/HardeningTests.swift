...
struct LifecycleHardeningTests {
    @Test func expiredPassingVoteCreatesOneScheduledChallenge() {
        ...
        let first = CahootsStateReconciler.reconcile(snapshot: snapshot, at: now)
        let second = CahootsStateReconciler.reconcile(snapshot: first.snapshot, at: now)
    ...
    @Test func reconcileClearsStaleStreakWhenUserHasNoAcceptedCheckIns() {
        ...
        var snapshot = DemoSeed.make(now: now)
        let lunchChallenge = snapshot.challenges.first { $0.title == "Lunch Walk Challenge" }
        guard let challenge = lunchChallenge else {
            Issue.record("Missing Lunch Walk Challenge")
            return
        }
        ...
        let cleared = CahootsStateReconciler.reconcile(snapshot: snapshot, at: now)
        ...
        let completed = CahootsStateReconciler.reconcile(snapshot: afterCheckIn, at: now)
...
struct MultiGroupAndRoutingTests {
    ...
    @Test func parsesUniversalAndCustomInviteRoutes() {
        #expect(AppRoute.parse(URL(string: "https://invite.cahoots.test/join/abc123")!, inviteHost: "invite.cahoots.test") == .joinGroup(code: "ABC123"))
        #expect(AppRoute.parse(URL(string: "cahoots://join/abc123")!, inviteHost: "invite.cahoots.test") == .joinGroup(code: "ABC123"))
        #expect(AppRoute.parse(URL(string: "https://evil.test/join/abc123")!, inviteHost: "invite.cahoots.test") == nil)
        #expect(AppRoute.parse(URL(string: "cahoots://join/no")!, inviteHost: "invite.cahoots.test") == nil)
...
@MainActor
private final class AuthRequiredRepository: AppRepository {
    var mode: AppMode = .demo
...
struct SupabaseTransportTests {
    ...
    @MainActor
    @Test func appleIdTokenExchangeIncludesNonce() async throws {
        StubURLProtocol.reset(with: [(200, #"{"access_token":"access","refresh_token":"refresh","expires_in":3600}"#)])
        ...
        // Keychain writes are unavailable in the unit-test host (-34018); still verify the outbound body.
        _ = try? await auth.exchangeAppleIdentityToken("id-token", nonce: "raw-nonce-value")
        let body = try #require(StubURLProtocol.lastHTTPBody)
        let json = try #require(JSONSerialization.jsonObject(with: body) as? [String: Any])
        #expect(json["nonce"] as? String == "raw-nonce-value")
        #expect(json["provider"] as? String == "apple")
        #expect(json["id_token"] as? String == "id-token")
        #expect(StubURLProtocol.requestCount == 1)
    }
}
...
struct SessionRecoveryTests {
    @MainActor
    @Test func authenticationRequiredLoadSignsTheUserOut() async {
        UserDefaults.standard.set(true, forKey: AppDefaults.onboardingComplete)
        defer { UserDefaults.standard.set(false, forKey: AppDefaults.onboardingComplete) }

        let repository = AuthRequiredRepository()
...
struct BackendErrorCopyTests {
    ...
    @Test func unrecognisedServerTextIsPassedThrough() {
        let message = "Cahoots is temporarily unavailable."
...
final class StubURLProtocol: URLProtocol {
    ...
    static var lastHTTPBody: Data? { lock.withLock { lastBody } }
    ...
    override func startLoading() {
        let entry: (status: Int, body: String) = Self.lock.withLock {
            Self.lastBody = request.httpBody ?? Self.readHTTPBodyStream(from: request)
...
struct LiveOfflineOverlayTests {
    @MainActor
    @Test func mergeKeepsLocalPendingSubmissionsMissingFromServer() {
        let now = Date.now
        var server = SnapshotMigrator.migrate(DemoSeed.make(now: now))
        server.pendingOperations = []
        let localID = UUID()
        let localSubmission = Submission(
            id: UUID(),
            clientGeneratedID: localID,
            challengeID: server.challenges[0].id,
            userID: server.currentUser.id,
            requirementDate: Calendar.current.startOfDay(for: now),
            quantity: 20,
            measurementType: .repetitions,
            completedAt: now,
            submittedAt: now,
            syncState: .waiting,
            verificationState: .honourSystem,
            createdAt: now,
            updatedAt: now
        )
        ...
        let merged = LocalSnapshotStore.merge(server: server, overlay: overlay)
        #expect(merged.pendingOperations.contains { $0.clientGeneratedID == localID })
        #expect(merged.submissions.contains { $0.clientGeneratedID == localID })
    }

    @Test func rateLimitedErrorIsReadable() {
        #expect(RepositoryError.readable("rate_limited") == "Too many attempts. Wait a moment and try again.")
        #expect(RepositoryError.emailConfirmationRequired.errorDescription == "Confirm your email before signing in.")
    }
}
