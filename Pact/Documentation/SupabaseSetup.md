...
2. Applied to the `pactapp` project via the Supabase MCP `apply_migration` tool (or `supabase db push` when the CLI is linked):
...
3. Confirm RLS is enabled on every public table and run the Security Advisor.
4. Deploy Edge Functions with JWT verification enabled (except `purge-workout-clips`, which uses `x-cron-secret`):
   - `app-snapshot`
   - `submit-workout`
   - `finalize-vote`
   - `clip-upload-url`
...
1. In Apple Developer, enable Sign in with Apple for App ID `com.callumoconnor.round`.
...
- `purge-expired-workout-clips` daily at 03:00 UTC
