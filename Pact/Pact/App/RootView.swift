...
struct RootView: View {
    ...
    var body: some View {
        ...
        .alert("Your session has expired", isPresented: Bindable(store).sessionRecoveryRequired) {
            Button("Sign in again") { store.beginSessionRecovery() }
...
private struct LoadRecoveryView: View {
    ...
    var body: some View {
        VStack(spacing: AppSpacing.large) {
            RoundEmptyState(symbol: "externaldrive.badge.exclamationmark", title: "Couldn’t load Round", message: message)
            ...
            if store.mode == .demo {
                ...
                Text("Resetting is explicit. Round does not replace unreadable saved data automatically.")
    ...
    var body: some View {
        VStack(spacing: AppSpacing.large) {
            ...
            VStack(spacing: AppSpacing.small) {
                ...
                Text("Round can remind you about scheduled check-ins and votes. No workout quantities appear on your lock screen.")
