...
final class OfflineSyncCoordinator {
    ...
    private let retryDelays: [TimeInterval] = [5, 30, 120, 600, 1_800]
    ...
    func drain(_ source: DemoSnapshot, connected: Bool, force: Bool = false) async -> DemoSnapshot {
        ...
        while let operation = nextOperation(in: snapshot, force: force, excluding: attemptedOperationIDs) {
            ...
            let result = await repository.syncSubmission(submission)
    ...
    private func apply(
        ...
        case .retryable(let reason, let retryAfter):
    ...
    private func projectedStreak(userID: UUID, challenge: RoundChallenge, snapshot: DemoSnapshot) -> Int {
