...
enum AppColors {
    ...
    static let brand = Color(red: 0.18, green: 0.72, blue: 0.43)
    static let accent = Color(uiColor: UIColor { traits in
        if traits.accessibilityContrast == .high {
            return traits.userInterfaceStyle == .dark
                ? UIColor(red: 0.43, green: 0.90, blue: 0.64, alpha: 1)
                : UIColor(red: 0.03, green: 0.36, blue: 0.18, alpha: 1)
        }
        return traits.userInterfaceStyle == .dark
            ? UIColor(red: 0.30, green: 0.85, blue: 0.55, alpha: 1)
            : UIColor(red: 0.04, green: 0.44, blue: 0.22, alpha: 1)
    })
...
struct RoundCard<Content: View>: View {
    let elevated: Bool
...
struct SecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .padding(.horizontal, AppSpacing.medium)
            .background(AppColors.card.opacity(configuration.isPressed ? 0.65 : 1), in: Capsule())
    }
}
...
struct RoundSectionHeader: View {
    let title: String
    var action: String?
    var onAction: (() -> Void)?
