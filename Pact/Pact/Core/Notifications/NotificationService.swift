...
enum RoundNotificationKind: String, Codable, Sendable {
...
struct NotificationPlanItem: Identifiable, Hashable, Sendable {
    var id: String
    var kind: RoundNotificationKind
    var fireDate: Date
    var groupID: UUID
...
actor NotificationService: NotificationScheduling {
    private let identifierPrefix = "round."
    ...
    func replacePlan(_ plan: [NotificationPlanItem]) async {
        ...
        let existing = await center.pendingNotificationRequests().map(\.identifier).filter { $0.hasPrefix(identifierPrefix) }
    ...
    private func content(for kind: RoundNotificationKind) -> UNMutableNotificationContent {
        let content = UNMutableNotificationContent()
        content.sound = .default
        switch kind {
        case .daily:
            content.title = String(localized: "Your Round check-in")
        ...
        case .deadline:
            content.title = String(localized: "Today’s Round closes soon")
...
enum NotificationPlanBuilder {
    ...
    private static func append(
        ...
        candidates.append(.init(
            id: "round.\(kind.rawValue).\(groupID.uuidString).\(subject).\(dateToken)",
