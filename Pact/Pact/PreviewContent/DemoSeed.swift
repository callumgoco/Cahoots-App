...
enum DemoSeed {
    static func make(
        ...
        let you = RoundUser(id: UUID(uuidString: "00000000-0000-0000-0000-000000000001") ?? UUID(), appleSubjectID: nil, displayName: "Alex Chen", avatarPath: nil, timezoneIdentifier: "Europe/London", createdAt: createdAt, updatedAt: now, deletedAt: nil, showsExactTotals: false)
        ...
        let group = RoundGroup(id: groupID, name: "Saturday Crew", emoji: "⚡️", ownerID: jennifer.id, memberLimit: 12, createdAt: createdAt, updatedAt: now, archivedAt: nil)
        ...
        let challenge = RoundChallenge(id: challengeID, groupID: groupID, proposalID: nil, title: "30-Day Push-Up Round", activityType: "push-ups", measurementType: .repetitions, minimumQuantity: 15, frequencyType: .daily, scheduledWeekdays: Set(1...7), timesPerWeek: nil, startDate: start, endDate: end, challengeTimezone: "Europe/London", dailyDeadlineMinutes: saturdayDeadlineMinutes, recoveryDayAllowance: 2, status: .active, scoringVersion: 1, createdAt: start)

        let secondGroupID = UUID(uuidString: "10000000-0000-0000-0000-000000000002") ?? UUID()
        let secondGroup = RoundGroup(id: secondGroupID, name: "Lunch Break Club", emoji: "🌿", ownerID: you.id, memberLimit: 8, createdAt: createdAt.addingTimeInterval(-86_400), updatedAt: now, archivedAt: nil)
        ...
        let secondChallenge = RoundChallenge(id: secondChallengeID, groupID: secondGroupID, proposalID: nil, title: "Lunch Walk Round", activityType: "walking minutes", measurementType: .minutes, minimumQuantity: 20, frequencyType: .selectedWeekdays, scheduledWeekdays: [2, 3, 4, 5, 6], timesPerWeek: nil, startDate: start, endDate: end, challengeTimezone: "Europe/London", dailyDeadlineMinutes: 14 * 60, recoveryDayAllowance: 1, status: .active, scoringVersion: 1, createdAt: start)
        ...
        let secondActivity: [ActivityFeedItem] = [
            ...
            item(secondGroupID, you, .challengeStarted, "Lunch Walk Round has started.", start)
        ]

        let invite = GroupInvite(id: UUID(), groupID: groupID, code: "ROUND6", createdBy: jennifer.id, expiresAt: now.addingTimeInterval(14 * 86_400), maximumUses: 12, useCount: 6, revokedAt: nil)
    ...
    private static func user(_ name: String, index: Int, createdAt: Date, now: Date) -> RoundUser {
        let id = UUID(uuidString: String(format: "00000000-0000-0000-0000-%012d", index)) ?? UUID()
        return RoundUser(id: id, appleSubjectID: nil, displayName: name, avatarPath: nil, timezoneIdentifier: "Europe/London", createdAt: createdAt, updatedAt: now, deletedAt: nil, showsExactTotals: false)
    }

    private static func item(_ groupID: UUID, _ actor: RoundUser, _ type: ActivityEventType, _ message: String, _ date: Date) -> ActivityFeedItem {
