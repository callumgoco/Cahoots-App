...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .confirmationDialog("Sign out of Round?", isPresented: $showSignOut) { Button("Sign out", role: .destructive) { store.signOut() } }
        ...
        RoundCard(elevated: true) {
            AdaptiveStack(spacing: AppSpacing.medium) {
                ...
                VStack(alignment: .leading, spacing: AppSpacing.micro) {
                    Text(store.currentUser?.displayName ?? "Round member").font(.title.bold())
    ...
    private func groupRow(_ group: RoundGroup) -> some View {
    ...
    private func role(_ group: RoundGroup) -> GroupRole { store.snapshot?.memberships.first { $0.groupID == group.id && $0.userID == store.currentUser?.id && $0.status == .active }?.role ?? .member }
        ...
        NavigationStack {
            Form { TextField("Display name", text: $name).textInputAutocapitalization(.words) }
                ...
                .toolbar {
                    ...
                    ToolbarItem(placement: .confirmationAction) { Button("Save") { Task { await store.updateDisplayName(name); dismiss() } }.disabled(TextSanitizer.clean(name).count < 2) }
                }
                .keyboardDoneToolbar()
...
struct NotificationSettingsView: View {
    ...
    var body: some View {
            ...
            if settings != nil {
                ...
                ForEach(store.activeGroups) { group in
                    Section("\(group.emoji) \(group.name)") {
                        ...
                        Toggle("Round updates", isOn: groupBinding(group.id, \.challengeUpdatesEnabled))
                ...
                Section("Quiet hours") {
                    ...
                    Text("Round never includes detailed workout quantities in lock-screen notifications.").font(.caption).foregroundStyle(AppColors.secondaryInk)
                }
            }
        }
        ...
        .confirmationDialog("Leave without saving?", isPresented: $confirmLeaveAfterFailure, titleVisibility: .visible) {
            ...
            Text("Round couldn’t save your latest notification changes. Keep editing to try again.")
        }
        .onAppear {
    ...
    private func challenge(for groupID: UUID) -> RoundChallenge? {
