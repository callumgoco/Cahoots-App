...
struct TodayView: View {
    ...
    private var emptyRoundState: some View {
        VStack(spacing: AppSpacing.large) {
            ...
            if store.currentProposal != nil {
                ...
                Button("Start a round") { showBuilder = true }
                    .buttonStyle(PrimaryButtonStyle())
                    .accessibilityIdentifier("today.startRound")
            }
    ...
    private func posterCard(_ challenge: RoundChallenge) -> some View {
        let requirementState = state(for: challenge)
        return RoundCard(elevated: true) {
            VStack(alignment: .leading, spacing: AppSpacing.large) {
                ...
                case .upcoming:
                    Label("Round starts soon", systemImage: "calendar.badge.clock")
                        .font(.title2.bold())
    ...
    private func heroDeadlineCountdown(_ challenge: RoundChallenge) -> some View {
                                    ...
                                    VStack(spacing: 4) {
                                        ...
                                        if spoilered {
                                            Text("Hidden")
                                                .font(.caption2)
                                                .foregroundStyle(AppColors.secondaryInk)
                                        } else if let challenge = store.currentChallenge {
    ...
    private func captionRow(_ challenge: RoundChallenge) -> some View {
    ...
    private func weekTokens(for challenge: RoundChallenge) -> [WeekDayToken] {
    ...
    private func state(for challenge: RoundChallenge) -> TodayRequirementState {
    ...
    private func accessibilitySummary(_ challenge: RoundChallenge, state: TodayRequirementState) -> String {
        switch state {
        ...
        case .upcoming: "Round has not started. Starts \(challenge.startDate.formatted(date: .long, time: .omitted))."
