...
function mapUser(row: Record<string, unknown>) {
  return {
    id: row.id, appleSubjectID: row.apple_subject_id ?? null,
...
Deno.serve(async (request) => {
  ...
  // Authoritative spoiler gating via SQL helper (challenge timezone + deadline).
  const revealBySubmissionID = new Map<string, boolean>();
  await Promise.all(submissionRows.map(async (row) => {
    if (row.user_id === userID) {
      revealBySubmissionID.set(row.id, true);
      return;
    }
    const { data } = await client.rpc("can_reveal_submission_id", { submission_id_input: row.id });
    revealBySubmissionID.set(row.id, data === true);
  }));
  ...
  return json({
    ...
    activity: (activityResult.data ?? []).map((row) => ({ id: row.id, groupID: row.group_id, actorID: row.actor_id, actorName: row.actor_id ? (userByID.get(row.actor_id)?.displayName ?? "A member") : "Round", eventType: activityEvent(row.event_type), message: row.message, createdAt: iso(row.created_at) })),
